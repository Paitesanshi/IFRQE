...
from IFQRE.data.dataset import Dataset
from IFQRE.utils import FeatureSource, FeatureType, set_color
from IFQRE.utils.url import decide_download, download_url, extract_zip


class KnowledgeBasedDataset(Dataset):
    """:class:`KnowledgeBasedDataset` is based on :class:`~IFQRE.data.dataset.dataset.Dataset`,
    ...
    def _download(self):
        ...
        if decide_download(url):
            ...
            self.logger.info(
                ...
                f'You can run IFQRE after the conversion, see you soon.'
            )
            exit(0)
        else:
