...
from IFQRE.utils import FeatureSource, FeatureType, get_local_time, set_color, ensure_dir
from IFQRE.utils.url import decide_download, download_url, extract_zip, makedirs, rename_atomic_files
...
class Dataset:
    ...
    imputation. Features are stored as :class:`pandas.DataFrame` inside :class:`~IFQRE.data.dataset.dataset.Dataset`.
    General and Context-aware Models can use this class.

    By calling method :meth:`~IFQRE.data.dataset.dataset.Dataset.build`, it will processing dataset into
    DataLoaders, according to :class:`~IFQRE.config.eval_setting.EvalSetting`.
    ...
    Attributes:
        ...
        field2type (dict): Dict mapping feature name (str) to its type (:class:`~IFQRE.utils.enum_type.FeatureType`).

        field2source (dict): Dict mapping feature name (str) to its source
            (:class:`~IFQRE.utils.enum_type.FeatureSource`).
    ...
    def _data_filtering(self):
        ...
        Note:
            ...
            thus :meth:`~IFQRE.data.dataset.dataset.Dataset._reset_index` will reset the index of feats.
        """
    ...
    def _load_user_or_item_feat(self, token, dataset_path, source, field_name):
        ...
        Note:
            ``user_id`` and ``item_id`` has source :obj:`~IFQRE.utils.enum_type.FeatureSource.USER_ID` and
            :obj:`~IFQRE.utils.enum_type.FeatureSource.ITEM_ID`
        """
    ...
    def _fill_nan(self):
        ...
        For fields with type :obj:`~IFQRE.utils.enum_type.FeatureType.TOKEN`, missing value will be filled by
        ``[PAD]``, which indexed as 0.

        For fields with type :obj:`~IFQRE.utils.enum_type.FeatureType.FLOAT`, missing value will be filled by
    ...
    def float_like_fields(self):
        """Get fields of type :obj:`~IFQRE.utils.enum_type.FeatureType.FLOAT` and
        :obj:`~IFQRE.utils.enum_type.FeatureType.FLOAT_SEQ`.
    ...
    def token_like_fields(self):
        """Get fields of type :obj:`~IFQRE.utils.enum_type.FeatureType.TOKEN` and
        :obj:`~IFQRE.utils.enum_type.FeatureType.TOKEN_SEQ`.
    ...
    @property
    def seq_fields(self):
        """Get fields of type :obj:`~IFQRE.utils.enum_type.FeatureType.TOKEN_SEQ` and
        :obj:`~IFQRE.utils.enum_type.FeatureType.FLOAT_SEQ`.
    ...
    def non_seq_fields(self):
        """Get fields of type :obj:`~IFQRE.utils.enum_type.FeatureType.TOKEN` and
        :obj:`~IFQRE.utils.enum_type.FeatureType.FLOAT`.
    ...
    def _split_index_by_leave_one_out(self, grouped_index, leave_one_num):
        ...
        for index in grouped_index:
            ...
            if legal_leave_one_num==0:
                next_index[1].extend(index[:pr])
                next_index[2].extend(index[:pr])
            elif legal_leave_one_num==1:
                next_index[1].append(index[-1])
    ...
    def build(self):
        ...
        See :class:`~IFQRE.config.eval_setting.EvalSetting` for details.
    ...
    def _dataframe_to_interaction(self, data):
        """Convert :class:`pandas.DataFrame` to :class:`~IFQRE.data.interaction.Interaction`.

        Args:
            data (pandas.DataFrame): data to be converted.

        Returns:
            :class:`~IFQRE.data.interaction.Interaction`: Converted data.
        """
        new_data = {}
        for k in data:
            value = data[k].values
