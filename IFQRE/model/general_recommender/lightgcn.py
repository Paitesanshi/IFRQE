...
from IFQRE.model.init import xavier_uniform_initialization
from IFQRE.model.loss import BPRLoss, EmbLoss
...
class LightGCN(GeneralRecommender):
    ...
    def calculate_loss(self, interaction):
        ...
        predict = torch.mul(u_embeddings, i_embeddings).sum(dim=1)

        predict=self.sigmoid(predict)
        loss = self.mf_loss(predict, label)
        return loss
