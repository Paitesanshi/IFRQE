...
from IFQRE.evaluator import metric_types, smaller_metrics
from IFQRE.utils import get_model, Enum, EvaluatorType, ModelType, InputType, \
...
class Config(object):
    ...
    Configurator module will first load the default parameters from the fixed properties in IFQRE and then
        ...
        if file_list:
            for file in file_list:
                with open(file, 'r', encoding='utf-8') as f:
                    file_config_dict.update(yaml.load(f, Loader=self.yaml_loader))
    ...
    def _load_cmd_line(self):
        ...
        if len(unrecognized_args) > 0:
            logger = getLogger()
            logger.warning('command line args [{}] will not be used in IFQRE'.format(' '.join(unrecognized_args)))
